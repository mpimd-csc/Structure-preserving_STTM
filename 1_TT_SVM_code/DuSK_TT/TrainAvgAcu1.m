function [Bestcv, Bestc,Bestg,time_tr,time_te]= TrainAvgAcu1(X,label,R,k,data_TT2,choose,c1,c2,g1,g2);
...
%          R :       the rank of tensor-train decomposition
...
for log2g = g1:g2 
    for cv=1:k
        ...
        for p=1:length(b)
            for q=1:p
                if Order~=1   
                    Ktrain(p,q)=Ker_DuSK(data_TT2{b(p),1},data_TT2{b(q),1},Order,R,flagker,2^log2g);
                else
        ...
        for r=1:N-length(b)
            for p=1:length(b)
                if Order~=1
                    Ktest(r,p)=Ker_DuSK(data_TT2{choose{cv,1}(1,r),1},data_TT2{b(p),1},Order,R,flagker,2^log2g);
                else
